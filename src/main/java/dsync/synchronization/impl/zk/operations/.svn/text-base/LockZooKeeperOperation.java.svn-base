...
public class LockZooKeeperOperation implements ZooKeeperOperation {
    ...
    public LockZooKeeperOperation(boolean isBlocking, OperationConfig config) {
...
/*        for (String name : names) {
    ...
    public boolean execute() throws KeeperException, InterruptedException {
        do {
            ...
            String prefix = "x--" + ip + "-" + vmProcess + "-" + Thread.currentThread().getId() +
                            "--";
            //System.out.println("Current thread id:" + Thread.currentThread().getId());
            // lets try look up the current ID if we failed
            // in the middle of creating the znode
            findPrefixInChildren(prefix, config.getZookeeper(), config.getDir());
            ZNodeName idName = new ZNodeName(threadLocalId.get().toString());
                ...
                if (names.isEmpty()) {
                    ...
                    if (!lessThanMe.isEmpty()) {
                        ...
                        synchronized (idName.getName().intern()) {
                            Stat stat = config.getZookeeper().
                                    exists(lastChildId, new LockWatcher(idName.getName().intern()));
                            if (stat != null) {
...
*/
    ...
    public boolean isOwner(String ownerId) {
        return threadLocalId.get() != null && ownerId != null && threadLocalId.get().toString().
                equals(ownerId);
    }
