...
public class ReadLockZooKeeperOperation implements ZooKeeperOperation {

    private static final Logger log = Logger.getLogger(ReadLockZooKeeperOperation.class);

    private static final int WAIT_TIME = 1000;

    boolean isBlocking;
    OperationConfig config;
    ThreadLocal<String> threadLocalId;

    public ReadLockZooKeeperOperation(boolean isBlocking, OperationConfig config) {
        this.isBlocking = isBlocking;
        this.config = config;
        this.threadLocalId = config.getThreadLocalId();
    }
    ...
    private void findPrefixInChildren(String prefix, ZooKeeper zookeeper, String dir)
        ...
        for (String name : names) {
            if (name.startsWith(prefix)) {
                threadLocalId.set(dir + "/" + name);
                //id = dir + "/" + name;
                if (log.isDebugEnabled()) {
                    log.debug("Found id created last time: " + threadLocalId.get());
                }
                break;
            }
        }
        if (threadLocalId.get() == null) {
            ...
            threadLocalId.set(idStr);

            if (log.isDebugEnabled()) {
                log.debug("Created id: " + threadLocalId.get());
    ...
    */
    ...
    public boolean execute() throws KeeperException, InterruptedException {
        do {
            ...
            String prefix = "read--" + ip + "-" + vmProcess + "-" + Thread.currentThread().getId() +
                            "--";
            ...
            if (threadLocalId.get() != null) {
                ZNodeName idName = new ZNodeName(threadLocalId.get());
                ...
                if (names.isEmpty()) {
                    ...
                    int mySequence = idName.getSequence();
                    ZNodeName lastZname = null;
                    String lesserChild = null;

                    if (sortedNames.size() > 0) {

                        if (sortedNames.last().getSequence() < mySequence) {
                            lesserChild = sortedNames.last().getName();
                        } else if (sortedNames.first().getSequence() > mySequence) {
                            // If all the write node sequences are greater current read node should
                            // be given the lock
                            lesserChild = null;
                        } else {
                            for (ZNodeName zName : sortedNames) { // If my sequence is in the middle
                                int sequence = zName.getSequence();
                                if (mySequence < sequence) {
                                    lesserChild = lastZname.getName();
                                    break;
                                } else if (mySequence == sequence) {
                                    lesserChild = zName.getName();
                                    break;
                                }
                    ...
                    if (lesserChild != null) {
                        if (log.isDebugEnabled()) {
                            log.debug("watching less than me node: " + lesserChild);
                        }

                        synchronized (idName.getName().intern()) {
                            Stat stat = config.getZookeeper().exists(lesserChild,
                                                                     new LockWatcher(
                                                                             idName.getName().
                            ...
                            if (stat != null) {
                                ...
                                // lesserChild node already gotten deleted before the wait, 
                                ...
                                // deleted.
                                String lesserNode = getNodeName(lesserChild);
                                while (nodeExists(lesserNode)) {
                                    if (isBlocking) {
                                ...
                                log.warn("Could not find the" +
                                         " stats for less than me: " + lesserChild);
                                threadLocalId.set(null);
                            }
                        }
                    } else {
